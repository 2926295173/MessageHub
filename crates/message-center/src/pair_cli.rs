...
//! connection to the message-center, sends `device.hello`, then waits up to 10s
...
use crate::identity::CenterIdentity;

pub async fn run(peer: SocketAddr, identity: CenterIdentity, _config: Arc<Config>) -> Result<()> {
    ...
    if got_pair_request {
        ...
        println!("WS_OK: hello accepted (message-center logged session); no pair.request observed (expected without web console trigger)");
    }
