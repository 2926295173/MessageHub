...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/pair/incoming", get(list_incoming_pairings))
        .route("/pair/incoming-accept", post(incoming_accept))
        .route("/pair/incoming-reject", post(incoming_reject))
...
#[utoipa::path(
    ...
    tag = "health",
    responses(
        (status = 200, description = "MessageCenter is healthy", body = HealthBody),
    )
)]
...
#[utoipa::path(
    ...
    tag = "identity",
    responses(
        (status = 200, description = "This message-center’s cert + identity", body = CertBody),
    )
)]
...
pub struct PairAcceptRequest {
    ...
    pub device_id: Uuid,
    /// The 4-digit code the user typed (visual confirmation only). Optional
    /// in MVP: the message-center currently does not re-derive the code (KDF
...
/// 4-digit code on the desktop and confirmed it matches what Android
/// shows.
#[utoipa::path(
    post,
...
async fn pair_accept(
    ...
    // Optional: validate the 4-digit code shape (matches the protocol
...
// ============================================================================
// Incoming pairings (phone-initiated, no-code)
// ============================================================================

#[derive(Debug, Serialize, ToSchema)]
struct PendingIncomingDto {
    device_id: Uuid,
    name: String,
    received_at: i64,
}

/// `GET /api/v1/pair/incoming` — list phone-initiated `device.pair.request`
/// envelopes that are awaiting user approval on the web console. The
/// flow is no-code by design: the phone is the trusted UI surface, the
/// user simply identifies the phone by `device_id` and clicks Accept.
#[utoipa::path(
    get,
    path = "/api/v1/pair/incoming",
    tag = "pairings",
    responses(
        (status = 200, description = "List of pending pair requests"),
    )
)]
async fn list_incoming_pairings(State(state): State<AppState>) -> impl IntoResponse {
    let items: Vec<PendingIncomingDto> = state
        .pending_incoming
        .list()
        .into_iter()
        .map(|p| PendingIncomingDto {
...
#[derive(Debug, Deserialize, ToSchema)]
pub struct IncomingAcceptRequest {
    /// The device id of the phone that sent the request.
    pub device_id: Uuid,
}

/// `POST /api/v1/pair/incoming-accept` — the user clicked "Accept" on
/// the web console. Sends `device.pair.confirm(accepted=true)` to the
/// phone. No `device.pair.challenge` is generated — this is the
/// no-code phone-initiated path.
#[utoipa::path(
    post,
    path = "/api/v1/pair/incoming-accept",
    tag = "pairings",
    request_body = IncomingAcceptRequest,
    responses(
        (status = 200, description = "device.pair.confirm(true) sent"),
        (status = 404, description = "No pending request for this device"),
        (status = 503, description = "Device not connected"),
    )
)]
async fn incoming_accept(
    State(state): State<AppState>,
    Json(req): Json<IncomingAcceptRequest>,
) -> impl IntoResponse {
    let device_id = req.device_id;
    // Drop the pending entry first; we own the decision now.
    let pending = match state.pending_incoming.remove(&device_id) {
        Some(p) => p,
        None => {
            return (
                StatusCode::NOT_FOUND,
                Json(json!({
                    "error": "no pending pair request for this device",
                    "device_id": device_id,
                })),
            )
                .into_response();
    ...
    if !state.registry.connected_ids().contains(&device_id) {
        // The phone disconnected between request and accept. Nothing to do.
        return (
            StatusCode::SERVICE_UNAVAILABLE,
            Json(json!({
                "error": "device not connected",
    ...
    let env = match phonebridge_proto::Envelope::new(
        ...
        state.our_device_id,
        phonebridge_proto::PairConfirm { accepted: true },
    ) {
        Ok(e) => e,
        Err(e) => {
            return (
    ...
    info!(%device_id, name = %pending.name, "incoming_accept: sent device.pair.confirm(true)");

    (
        StatusCode::OK,
        Json(json!({
            "status": "pair.confirm(true) sent",
            "device_id": device_id,
        })),
    )
        .into_response()
...
/// `POST /api/v1/pair/incoming-reject` — the user clicked "Reject" on
/// the web console. Sends `device.pair.confirm(accepted=false)` and
/// drops the pending entry.
#[utoipa::path(
    post,
    path = "/api/v1/pair/incoming-reject",
    tag = "pairings",
    request_body = IncomingAcceptRequest,
    responses(
        (status = 200, description = "device.pair.confirm(false) sent"),
        (status = 404, description = "No pending request for this device"),
    )
)]
async fn incoming_reject(
    State(state): State<AppState>,
    Json(req): Json<IncomingAcceptRequest>,
) -> impl IntoResponse {
    let device_id = req.device_id;
    ...
    // Even if the device is no longer connected, dropping the entry
    // is enough — the phone will time out waiting for the confirm.
    if state.registry.connected_ids().contains(&device_id) {
        let env = match phonebridge_proto::Envelope::new(
            MessageType::DevicePairConfirm,
            state.our_device_id,
            phonebridge_proto::PairConfirm { accepted: false },
        ) {
            Ok(e) => e,
            Err(er) => {
                warn!(%device_id, "incoming_reject: build envelope failed: {er}");
                return (
                    StatusCode::OK,
                    Json(json!({
                        "status": "pending entry dropped (build failed)",
                        "device_id": device_id,
                    })),
                )
                    .into_response();
        ...
        if let Err(e) = state.registry.try_send(device_id, env).await {
            warn!(%device_id, "incoming_reject send failed (continuing): {e}");
        }
    }
    info!(%device_id, name = %pending.name, "incoming_reject: sent device.pair.confirm(false) and dropped pending entry");

    (
        StatusCode::OK,
        Json(json!({
            "status": "pair.confirm(false) sent",
            "device_id": device_id,
        })),
    )
        .into_response()
...
/// notification as dismissed in the message-center DB and broadcast a
...
mod tests {
    ...
    async fn make_state() -> AppState {
        ...
        // The display-endpoint token isn't exercised by the
        // REST tests, so we point it at a throwaway tmp dir
        // (no actual file is read; verify() is never called).
