...
//! messages and the CenterSink persists them to storage.
...
use message_center::center_sink::CenterSink;
...
/// A `WsSink` that wraps `CenterSink` and counts calls for assertions.
struct CountingSink {
    inner: Arc<CenterSink>,
...
impl WsSink for CountingSink {
    ...
    async fn on_notification_dismissed(
        ...
        self.inner
            .on_notification_dismissed(envelope_id, device_id, env)
            .await;
    }
    ...
    async fn on_sms_send_result(
        ...
        envelope_id: Uuid,
        device_id: Uuid,
        env: &phonebridge_proto::SmsSendResult,
    ) {
        self.inner
            .on_sms_send_result(envelope_id, device_id, env)
            .await;
    }
    async fn on_call_state(
        ...
        envelope_id: Uuid,
        device_id: Uuid,
        env: &phonebridge_proto::CallState,
    ) {
        self.inner.on_call_state(envelope_id, device_id, env).await;
    }
    ...
    async fn on_sms_list_result(
        ...
        envelope_id: Uuid,
        device_id: Uuid,
        env: &phonebridge_proto::SmsListResult,
    ) {
        self.inner
    ...
    async fn on_unpair(
        ...
        device_id: Uuid,
        env: &phonebridge_proto::Unpair,
    ) {
        self.inner.on_unpair(envelope_id, device_id, env).await;
    }
...
async fn ws_dispatch_persists_to_db() {
    ...
    let sink: Arc<dyn WsSink + Send + Sync> = Arc::new(CountingSink {
        inner: Arc::new(CenterSink::new(
            db_arc.clone(),
            message_center::console_bus::ConsoleBus::default(),
            message_center::display_bus::DisplayBus::default(),
        )),
...
/// Verify the test_context helper in the message-center crate produces a usable context.
#[tokio::test]
