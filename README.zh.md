...
- **开源 —— [GNU Affero 通用公共许可证 v3.0 或更新版本](LICENSE)**
  （`AGPL-3.0-or-later`）。完整文本在 [`LICENSE`](LICENSE)。
  您可以出于任何目的学习、修改、再分发本源码（源码或二进制形式），**条件是您分发的任何修改版本 —— 包括在网络服务器上运行的修改版本 —— 也必须在同样的 AGPL-3.0 条款下以完整的对应源代码形式提供**（参见 AGPL-3.0 第 13 条）。如果这符合您的使用场景，仅按 AGPL 即可。

- **商业许可证** —— 适用于需要以其他条款分发 PhoneBridge（或衍生作品）的组织：例如专有设备固件、OEM 捆绑、闭源设备，或不愿公开其修改的 SaaS 部署。详见 [`LICENSE-COMMERCIAL.md`](LICENSE-COMMERCIAL.md)，了解商业许可证通常涵盖的范围与申请方式。**在签署正式商业许可协议之前，该路径下的任何权利均不生效。** 在此之前，AGPL-3.0 适用。

如需开启商业授权洽谈，请提交带 `commercial-license` 标签的 GitHub Issue，或通过 [`docs/dev-setup.md`](docs/dev-setup.md) 中列出的渠道联系版权所有者。
